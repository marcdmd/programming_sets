...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    original_letter = alphabet.index(letter)  
...
def shift_by_letter(letter, letter_shift):
    ...
    letter_index = alphabet.index(letter_shift)  
    original_letter = alphabet.index(letter)  
    new_letter = (original_letter + letter_index) % 26  
...
def vigenere_cipher(message, key):
    ...
    for letter in message:
        if letter == " ":
            ...
            shift = ord(key[key_position]) - ord("A")
...
def scytale_cipher(message, shift):
    ...
    for i in range(message_length):
        encoded_message += message[(i // shift) + (message_length // shift) * (i % shift)]
